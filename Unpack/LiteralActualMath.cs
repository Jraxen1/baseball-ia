...
public class LiteralActualMath : MonoBehaviour
{
	...
	public bool isBackSpin = true;



	public bool BattingMode = false;
    ...
    void Update()
    {
		if (hasStarted)
		{
			...
			else
			{
				...
				else if (transform.position.x >= 18.44 || transform.position.y <= 0.2f && !BattingMode)
				{
					...
					if (BattingMode)
					{
						if (transform.position.y <= 0.2f){
							if (!hasFinished)
							{
								...
								if (BattingMode)
								{
									gameObject.GetComponent<HittableBall>().EndSim();
								}
